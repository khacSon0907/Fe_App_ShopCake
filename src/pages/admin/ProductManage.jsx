...
const ProductManage = () => {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        showNotification(
          response.data?.message || "Lỗi khi tải danh sách sản phẩm",
          "error"
        );
      }
    } catch (error) {
  ...
  const fetchCategories = async () => {
    try {
      ...
      if (response.success) {
        ...
        showNotification(
          response.data?.message || "Lỗi khi tải danh sách danh mục",
          "error"
        );
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (isNaN(weight) || weight < 10) {
        showNotification("Trọng lượng phải từ 10 gram trở lên", "error");
        return;
      }
      if (isNaN(price) || price <= 5000) {
        showNotification("Giá sản phẩm phải lớn hơn 5000đ", "error");
        return;
      }

      if (isNaN(stock) || stock < 20) {
        showNotification("Tồn kho phải từ 20 sản phẩm trở lên", "error");
        return;
      }
    ...
    if (window.confirm("Bạn có chắc chắn muốn xóa sản phẩm này?")) {
      try {
        ...
        if (response.success) {
          ...
          showNotification(
            response.data?.message || "Lỗi khi xóa sản phẩm",
            "error"
          );
        }
      } catch (error) {
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const matchCategory =
      selectedCategory === "" || product.categoryCode === selectedCategory;
    ...
    <Box sx={{ p: 3, backgroundColor: "#f5f7fa", minHeight: "100vh" }}>
      ...
      <Paper elevation={0} sx={{ mb: 3 }}>
        <Tabs
          ...
          textColor="primary"
        >
          <Tab icon={<ViewIcon />} label="Danh sách" iconPosition="start" />
          <Tab icon={<CategoryIcon />} label="Thống kê" iconPosition="start" />
        </Tabs>
              ...
              <Grid container spacing={2}>
                <Grid item xs={6}>
                  ...
                  <TextField
                    ...
                    required
                    error={
                      !!formData.price && parseFloat(formData.price) <= 5000
                    }
                    helperText={
                      !!formData.price && parseFloat(formData.price) <= 5000
                        ? "Giá phải lớn hơn 5000đ"
                        : ""
                    }
                    InputProps={{
                      startAdornment: (
                ...
                <Grid item xs={6}>
                  ...
                  <TextField
                    ...
                    margin="normal"
                    required
                    error={!!formData.stock && parseInt(formData.stock) < 20}
                    helperText={
                      !!formData.stock && parseInt(formData.stock) < 20
                        ? "Tồn kho phải ≥ 20"
                        : ""
                    }
                  />
